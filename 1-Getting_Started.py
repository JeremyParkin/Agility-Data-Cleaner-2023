...
if st.session_state.upload_step:
    ...
    col1, col2, col3 = st.columns(3) # [2, 3, 4, 4]
    with col1:
        ...
        # st.subheader(media_type_column)
        st.write(st.session_state.df_untouched[media_type_column].value_counts())
    with col2:
        st.subheader("Top Authors")
        # Filter df_untouched to only include specified media types
        # filtered_df = st.session_state.df_untouched[
        #     st.session_state.df_untouched[media_type_column].isin(['PRINT', 'ONLINE_NEWS', 'BLOG', 'PRESS_RELEASE'])]
        ...
        # st.session_state.original_top_authors = original_top_authors
    with col3:
...
if not st.session_state.upload_step:
    ...
    period = st.text_input('Reporting period or focus*', placeholder='eg. March 2022', key='period',
                                                      help='Required to build export file name.')
    uploaded_file = st.file_uploader(label='Upload your CSV or XLSX*', type=['csv', 'xlsx'], accept_multiple_files=False,
    ...
    if submitted and (client == "" or period == "" or uploaded_file is None):
        ...
        with st.spinner("Converting file format."):
            ...
            columns_to_change = ["Sentiment", "Continent", "Country", "Province/State", "Prov/State", "City", "Language"]
